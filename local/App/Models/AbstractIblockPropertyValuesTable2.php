 ...
 * @package Models
 */
abstract class AbstractIblockPropertyValuesTable2 extends DataManager
{
    //const IBLOCK_ID = null;

    /**
     * Потомок должен вернуть API_CODE инфоблока (например, 'Doctors')
     * @var string
     */
    ...
    /**
     * Общая логика получения ID для всех наследников
     * @return int
    ...
    public static function getIblockId(): int
    {
        ...
        if (!isset($ids[$apiCode])) {
            // Компилируем сущность и достаем ID
    ...
    public static function getMap(): array
    {
        ...
        $cacheDir = 'iblock_property_map/'.static::getIblockId();
    ...
    {
        if (isset(static::$properties[static::getIblockId()])) {
            return static::$properties[static::getIblockId()];
        }

        $dbResult = PropertyTable::query()
            ->setSelect(['ID', 'CODE', 'PROPERTY_TYPE', 'MULTIPLE', 'NAME', 'USER_TYPE'])
            ->where('IBLOCK_ID', static::getIblockId())
            ->exec();
        while ($row = $dbResult->fetch()) {
            static::$properties[static::getIblockId()][$row['CODE']] = $row;
        }
    ...
    public static function getEnumPropertyOptions(string $propertyCode, string $byKey = 'ID'): array
    {
        $dbResult = PropertyEnumerationTable::getList([
            ...
            'filter' => ['=PROPERTY.CODE' => $propertyCode, 'PROPERTY.IBLOCK_ID' => static::getIblockId()],
        ]);
        while ($row = $dbResult->fetch()) {
